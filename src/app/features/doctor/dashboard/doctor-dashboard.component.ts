  ...
  constructor(
    ...
    this.kpis$ = combineLatest([
      ...
      map(([a, p]): DashboardKpi[] => [
        { title: "Today's Appointments", value: a.Upcoming + a.Active, icon: 'fa-calendar-day', accent: 'sky', link: '/doctor/appointments' },
        { title: 'My Patients', value: p.Total, icon: 'fa-user-injured', accent: 'green', link: '/doctor/patients' },
        { title: 'Pending Reviews', value: 4, trend: 'Labs & imaging', icon: 'fa-file-medical', accent: 'yellow' },
        { title: 'Completed Today', value: a.Completed, icon: 'fa-circle-check', accent: 'green' },
      ])
    );
  }
}
